...
(() => {
  ...
  function disclosure(kind, rows) {
    ...
    button.addEventListener('click', () => {
      ...
      button.textContent = `관련 ${kind === 'projects' ? '프로젝트' : '논문'} ${expanded ? '접기 ↑' : '모두 보기 →'}`;
  ...
  const projects = relatedProjects.map(({ relation, project }, index) => {
    ...
    link.append(element('span', 'related-period', project.period), copy, element('span', 'related-cue', '프로젝트 보기 →'));
    return link;
  });
